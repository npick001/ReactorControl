...
function Logging.CheckDiskSpace()
	local driveFull = (fs.getFreeSpace("/") == 0)
	if driveFull then
		Logging.RefreshLogs()
	end
end
...
function Logging.OpenLogs()
	if not Logging.ERRORS then
		Logging.ERRORS = fs.open(err_log, "a")
	end
	if not Logging.DEBUG then
		Logging.DEBUG = fs.open(debug_log, "a")
	end
	if not Logging.INFO then
		Logging.INFO = fs.open(info_log, "a")
	end
end

function Logging.RefreshLogs()
	if Logging.ERRORS then
		Logging.ERRORS.close()
		Logging.ERRORS = fs.open(err_log, "w")
	end
	if Logging.DEBUG then
		Logging.DEBUG.close()
		Logging.DEBUG = fs.open(debug_log, "w")
	end
	if Logging.INFO then
		Logging.INFO.close()
		Logging.INFO = fs.open(info_log, "w")
	end
end

function Logging.LogDebug(message)
	Logging.CheckDiskSpace()
