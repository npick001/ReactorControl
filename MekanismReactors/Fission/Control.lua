...
function RC.Scram(r, status, msg, err_msg)
...
function RC.CheckCoolantLevels(r, status)
	...
	if coolant_level <= 20 then
		local msg = "ERROR: Reactor shut down due to low coolant"
		RC.Scram(r, status, msg, msg)
		return
	end
end
...
function RC.CheckHeatedCoolantLevels(r, status)
	...
	if heated_coolant_level >= 80 then
		local msg = "ERROR: Reactor shut down due to high heated coolant level"
		RC.Scram(r, status, msg, msg)
		return
	end
end
...
function RC.MitigateDamage(r, status)
	...
	if status.dmg_percent >= 80 then
		local msg = "ERROR: Reactor shut down due to critical damage"
		RC.Scram(r, status, msg, msg)
		return
	end
	...
	if status.dmg_percent >= 30 and status.dmg_percent <= 79 then
		local msg = "ERROR: Reactor shut down due to high damage"
		RC.Scram(r, status, msg, msg)
		return
	end
...
function RC.ControlTemperature(r, status)
	if status.heat >= 1000 then
		local msg = "ERROR: Reactor shut down due to high heat"
		RC.Scram(r, status, msg, msg)
		return
	end
end
...
function RC.MaintainFuelLevel(r, status)
	...
	if fuel_level <= 10 then
		local msg = "ERROR: Reactor shut down due to low fuel"
		RC.Scram(r, status, msg, "")
...
function RC.ManageWasteLevels(r, status)
	...
	if waste_level >= 80 then
		local msg = "ERROR: Reactor shut down due to high waste"
		RC.Scram(r, status, msg, msg)
...
function RC.TryReactivatingReactor(r, status)
	...
	if
		coolant_acceptable
		and fuel_acceptable
		and heated_coolant_acceptable
		and waste_acceptable
		and damage_acceptable
		and heat_acceptable
	then
		r.activate()
